%function [visited_cities, L] = NearestInsertionHeuristic(X, Y)
% solves the TSP with Nearest Insertion Heuristic
...
while length(visited_cities) < n
    % pick the closest unvisited city by finding the city with the minimum
    % distance to any of the visited cities
    Dd = D(visited_cities, unvisited_cities);
    [~, min_index] = min(Dd(:));
	[~, selected_city_ind] = ind2sub(size(Dd), min_index(1));
